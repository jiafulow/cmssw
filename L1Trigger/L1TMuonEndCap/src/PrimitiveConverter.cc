...
std::vector<ConvertedHit> PrimitiveConverter::convert(std::vector<TriggerPrimitive> TriggPrim, int SectIndex){
	...
	if(!IsNeighbor){
	  ...
	  //else
		  //std::cout<<BX-3<<" "<<endcap<<" "<<SectIndex + 1 - (endcap - 1)*6<<" "<<sub<<" "<<station<<" 1 "<<quality<<" "<<pattern<<" "<<wire<<" "<<C3.Id()<<" 0 "<<strip<<"\n";
	}
	else{
	
		  //std::cout<<BX-3<<" "<<endcap<<" "<<SectIndex + 1 - (endcap - 1)*6<<" "<<sub<<" 5 1 "<<quality<<" "<<pattern<<" "<<wire<<" "<<cindex+1<<" 0 "<<strip<<"\n";
	}
